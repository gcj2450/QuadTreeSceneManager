...
public class AutomaticLOD : MonoBehaviour
{
  ...
  [SerializeField]
  private SwitchMode m_switchMode = SwitchMode.SwitchMesh;
  ...
  private static void SetLODLevelsRecursive(AutomaticLOD root, GameObject gameObject)
  {
    ...
    if(bProcess && automaticLOD)
    {
      for (int i = 0; i < root.m_listLODLevels.Count; i++)
      {
        CheckForLODGameObjectSetup(root, automaticLOD, automaticLOD.m_listLODLevels[i], i);
      }
    }
  }
      ...
      if (IsRootOrBelongsToLODTree(automaticLOD, root))
      {
        ...
        if(bProcess)
        {
          ...
          if (bBefore)
          {
            ...
            if(bCreateMesh)
            {
              CheckForLODGameObjectSetup(root, automaticLOD, data, nLevel == 0 ? 0 : nLevel - 1);
            }
          }
          else
          {
            ...
            if(bCreateMesh)
            {
              CheckForLODGameObjectSetup(root, automaticLOD, data, nLevel == nLastLevel ? nLastLevel : nLevel + 1);
            }
          }
        }
      }
  ...
  private static void SwitchToLODRecursive(AutomaticLOD root, GameObject gameObject, int nLODLevel, bool bRecurseIntoChildren)
  {
    ...
    if (automaticLOD != null)
    {
      if (IsRootOrBelongsToLODTree(automaticLOD, root))
      {
        ...
        {
          if(root.m_switchMode == SwitchMode.SwitchMesh)
          {
          ...
          else if(root.m_switchMode == SwitchMode.SwitchGameObject)
          {
    ...
    if (automaticLOD != null)
    {
      if (IsRootOrBelongsToLODTree(automaticLOD, root))
      {
        if (automaticLOD.m_meshSimplifier != null)
        {
          ...
          CheckForLODGameObjectSetup(root, automaticLOD, automaticLOD.m_listLODLevels[nLevel], nLevel);
  ...
  private static void RestoreOriginalMeshRecursive(AutomaticLOD root, GameObject gameObject, bool bDeleteLODData, bool bRecurseIntoChildren)
  {
    ...
    if (automaticLOD != null)
    {
      if (IsRootOrBelongsToLODTree(automaticLOD, root))
      {
        ...
        if(root.m_switchMode == SwitchMode.SwitchGameObject)
        {
          if (gameObject.GetComponent<Renderer>() != null)
          {
